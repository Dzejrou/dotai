...
[GlobalClass]
public partial class TEST_sub_level : Spell
{
    ...
    public override bool TryCast(ISpellCaster caster, SpellCastRequest request)
    {
        ...
        if (player == null)
        {
            GD.PushWarning($"{GetPath()}: {nameof(TEST_sub_level)} can only be cast by a player.");
            return false;
        }
        ...
        {
            player.ShowFloatingText("MIN LEVEL", new Color(1.0f, 0.95f, 0.2f, 1.0f));
            return false;
        }
